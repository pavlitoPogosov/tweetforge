...
export function TweetGenerator() {
  ...
  const [error, setError] = useState("");aimport { Input } from "@/components/ui/input";
  import { Textarea } from "@/components/ui/textarea";
  import { Button } from "@/components/ui/button";
  import axios from "axios";
  import { Progress } from "@/components/ui/progress";
  ...
  // Initialize PostHog (this could be done in a higher-level component or in _app.js)
  posthog.init('your_posthog_api_key', { api_host: 'https://app.posthog.com' });
  
  const useProgressSteps = (loading: boolean, setProgress: (value: number) => void, setProgressText: (text: string) => void) => {
    useEffect(() => {
      const timeoutIds: NodeJS.Timeout[] = [];
  ...
  export function TweetGenerator() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      // Capture event when submit button is clicked
      posthog.capture('submit_button_clicked', { url });
      ...
      try {
        const response = await axios.post(
          "https://webhook.latenode.com/1194/prod/31c15231-66bf-47e9-a9e2-177154243a10",
          {
            url,
            instructions,
            personality: PERSONALITIES.find((x) => x.label === personality)?.prompt || "",
          }
        );
        ...
        if (Array.isArray(response.data)) {
          ...
          // Capture event when tweets are successfully loaded
          posthog.capture('tweets_loaded', { url });
        } else {
          setIsFormShown(true);
        ...
        toast.error("An error occurred while submitting the data. Please try again.");
        setIsFormShown(true);
      } finally {
        setLoading(false);
      }
    ...
    return (
      <div>
        <form onSubmit={handleSubmit} className="max-w-2xl mx-auto p-4 bg-white rounded" ref={formRef}>
          {isFormShown && (
            <>
              ...
              <p className="text-base sm:text-lg text-center text-gray-600 mb-2 md:mb-8">
                Give a URL to the content you like, add your suggestions, and get free, ready-to-post tweets.
              </p>
              ...
              <div className="mb-6">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    // Capture event when URL is entered
                    posthog.capture('url_entered', { url: e.target.value });
                  }}
        ...
        {!loading && !isFormShown && tweets.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl mx-auto p-4" ref={tweetsRef}>
            {tweets.map((tweet, index) => (
              <div key={index} className="p-4 bg-gray-100 rounded shadow-md flex flex-col justify-between">
                <p className="text-gray-800 mb-4">{tweet.text}</p>
                <CopyToClipboard
                  text={tweet.text}
                  onCopy={() => {
                    setCopiedIndex(index);
                    // Capture event when text is copied
                    posthog.capture('copy_text_clicked', { tweet_text: tweet.text });
                  }}
                >
